...
class ArmEnv(object):
    ...
    state_dim = 62  # Dimension of the state space
    ...
    def __init__(self, sim_port=23000):
        ...
        self.goal_pose = {
            'x': 1.00, 'y': 0.10, 'z': 0.70, 'pe': 0.05,  # Position and position error
            'alpha': 0, 'beta': 0, 'gamma': 0, 'oe': 15 * np.pi / 180  # Orientation and orientation error
        }
    ...
    def step(self, action):
        ...
        # Calculate reward
        r = -delta_pos*10-delta_orient*10  # Base reward based on position and orientation errors
        ...
        self.distance_new = delta_pos  # Update position error
        if self.distance_new < self.distance_old:
            r += 0.5  # Reward for reducing position error
        else:
            r -= 0.5  # Penalty for increasing position error
        self.distance_old = self.distance_new
        if distance_tipgoal < 0.15:
        ...
        for link_idx, sensors in enumerate(self.link_sensors):
            ...
            nearest_world_point = None
            # Traverse all sensors of this link to find nearest obstacle and calculate per-sensor obstacle avoidance reward
            for s in sensors:
                out = self.sim.handleProximitySensor(s)
                detected, dist = out[0], out[1]
                # ---- Original per-sensor obstacle avoidance reward logic ----
                if detected and dist < self.safe_distance:
                    if dist < 0.01:
                        r -= 10.0  # Large penalty for very close obstacles
                    else:
                        ...
                        if norm > 1e-6:
                            world_dir = [vx/norm, vy/norm, vz/norm]  # Normalize the vector
                        else:
                        ...
                        if dp > 0:
                            r += -1.0 / dist * dp / 20.0  # Penalty for moving towards obstacles
                        elif dp < 0:
                            r += 1.0 / dist * abs(dp) / 20.0  # Reward for moving away from obstacles
                # ---- END Original per-sensor obstacle avoidance reward logic ----
            ...
            # Calculate vector from link to nearest obstacle point
            if nearest_world_point is not None:
                link_pos = self.sim.getObjectPosition(self.links[link_idx], -1)
                vector = [nearest_world_point[i] - link_pos[i] for i in range(3)]
            else:
                vector = [0.0, 0.0, 0.0]
            # Get link motion direction
            link_dir = link_dirs[link_idx]
            # Append 7-dimensional information
            nearest_info.extend([
                vector[0], vector[1], vector[2],
                link_dir[0], link_dir[1], link_dir[2],
                min_dist/self.dist_norm
            ])
        # Replace original sensor_data
    ...
    def reset(self):
        ...
        """
        # Episode count and randomly switch predefined goal every 10 episodes
        ...
        self.arm_info = [0, 0, 0, -90 * np.pi / 180, 0, 90 * np.pi / 180, 0]  # Reset arm joint angles
