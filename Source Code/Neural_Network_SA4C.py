...
class SA4CAgent:
    def __init__(
        ...
        #Two more critic
        self.critic3 = QNetwork(state_dim, action_dim, hidden_dim).to(device)
        self.critic4 = QNetwork(state_dim, action_dim, hidden_dim).to(device)

        # Target networks
        self.target_critic1 = deepcopy(self.critic1).to(device)
        self.target_critic2 = deepcopy(self.critic2).to(device)
        self.target_critic3 = deepcopy(self.critic3).to(device)
        self.target_critic4 = deepcopy(self.critic4).to(device)

        # Optimizers
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=lr)
        self.critic1_optimizer = optim.Adam(self.critic1.parameters(), lr=lr)
        self.critic2_optimizer = optim.Adam(self.critic2.parameters(), lr=lr)
        self.critic3_optimizer = optim.Adam(self.critic3.parameters(), lr=lr)
        self.critic4_optimizer = optim.Adam(self.critic4.parameters(), lr=lr)
    ...
    def update(self, batch_size):
        ...
        with torch.no_grad():
            ...
            q3_next = self.target_critic3(next_states, next_actions)
            q4_next = self.target_critic4(next_states, next_actions)
            # Lowest critic value
            q_next_cat = torch.cat([q1_next, q2_next, q3_next, q4_next], dim=1)
            q_next_min = q_next_cat.min(dim=1, keepdim=True)[0]
            q_next = q_next_min - self.alpha * next_log_pi
        ...
        critic3_loss = F.mse_loss(q3, target_q)
        critic4_loss = F.mse_loss(q4, target_q)
        ...
        q3_pi = self.critic3(states, actions_pi)
        q4_pi = self.critic4(states, actions_pi)
        q_pi_cat = torch.cat([q1_pi, q2_pi, q3_pi, q4_pi], dim=1)
        q_pi = q_pi_cat.min(dim=1, keepdim=True)[0]
        ...
        for target_param, param in zip(self.target_critic3.parameters(), self.critic3.parameters()):
            target_param.data.copy_(target_param.data * (1 - self.tau) + param.data * self.tau)
        for target_param, param in zip(self.target_critic4.parameters(), self.critic4.parameters()):
            target_param.data.copy_(target_param.data * (1 - self.tau) + param.data * self.tau)

        # return loss
        return actor_loss.item(), critic1_loss.item(), critic2_loss.item(), critic3_loss.item(), critic4_loss.item()

    def save(self, save_dir="Models_SA4C"):
        ...
        torch.save(self.critic3.state_dict(), os.path.join(save_dir, "critic3.pth"))
        torch.save(self.critic4.state_dict(), os.path.join(save_dir, "critic4.pth"))
        torch.save(self.log_alpha,             os.path.join(save_dir, "log_alpha.pth"))

    def load(self, save_dir="Models_SA4C"):
        ...
        self.critic3.load_state_dict(torch.load(os.path.join(save_dir, "critic3.pth"), map_location=device))
        self.critic4.load_state_dict(torch.load(os.path.join(save_dir, "critic4.pth"), map_location=device))
    ...
    def evaluate_q(self, state, action):
        ...
        q3 = self.critic3(state, action)
        q4 = self.critic4(state, action)
        q_cat = torch.cat([q1, q2, q3, q4], dim=1)
        q_min = q_cat.min(dim=1, keepdim=True)[0]
        return q_min.item()
